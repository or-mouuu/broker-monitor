...
FRESH_PRIOR_WINDOW = 16    # 前期比較窗口（緊接在14日主窗口之前的交易日數）
FRESH_HOT_RATIO    = 2.0   # 近期買超頻率 ÷ 前期頻率 ≥2 → 升溫
FRESH_ROUTINE_FREQ = 0.7   # 前期買超頻率 ≥70% → 常態流量（前期本來就天天買）
FRESH_MIN_PRIOR_DAYS = 10  # 前期可比對天數低於此視為資料不足，一律 unknown
FRESHNESS_MULT = {"new": 1.10, "rising": 1.05, "routine": 0.85, "steady": 1.0, "unknown": 1.0}
...
def _hoard_prior_dates(ticker: str, twse_ohlc: dict, fallback_dates: list[str],
                       before_date: str, window: int) -> list[str]:
    """回傳緊接在 before_date 之前（不含）的 window 個交易日（舊→新）。
    供新鮮度維度比較「前期」買超頻率使用。"""
    price_days = sorted(d for d in twse_ohlc.get(ticker, {}) if d < before_date)
    if price_days:
        return price_days[-window:]
    local_days = sorted(d for d in fallback_dates if d < before_date)
    return local_days[-window:]


def _classify_freshness(recent_buy_days: int, recent_window_days: int,
                        prior_buy_days: int, prior_days: int) -> str:
    """依前期/近期買超頻率變化分類：new(剛開始吃) / rising(升溫) / routine(常態流量) / steady(持平)。
    前期可比對天數不足時回傳 unknown（不加不減分）。"""
    if prior_days < FRESH_MIN_PRIOR_DAYS:
        return "unknown"
    recent_freq = recent_buy_days / recent_window_days if recent_window_days else 0.0
    prior_freq  = prior_buy_days / prior_days if prior_days else 0.0
    if prior_freq < 0.2 and recent_freq >= 0.4:
        return "new"
    if recent_freq / max(prior_freq, 0.05) >= FRESH_HOT_RATIO:
        return "rising"
    if prior_freq >= FRESH_ROUTINE_FREQ:
        return "routine"
    return "steady"
...
def compute_hoarding(all_branches: list[dict], history: list[dict],
    ...
    for r in raw:
        ...
        # 新鮮度：比較主窗口(近14日) vs 前期(緊接其前16日)的買超頻率變化，區分「行為改變」與「例行流量」
        # 前期天數以「本地快照實際涵蓋」的日期計算，避免誤把「快照不足」當成「前期沒買」
        tk, br = r["ticker"], r["branch"]
        prior_dates_all = _hoard_prior_dates(tk, twse_ohlc, all_dates, trading_days[0], FRESH_PRIOR_WINDOW)
        prior_dates = [d for d in prior_dates_all if d in snap_idx]
        prior_buy_days = sum(
            1 for d in prior_dates
            if snap_idx.get(d, {}).get(br, {}).get(tk, {}).get("net", 0) > 0
        )
        freshness = _classify_freshness(r["buy_days"], len(trading_days), prior_buy_days, len(prior_dates))
        total_score = min(100.0, total_score * FRESHNESS_MULT.get(freshness, 1.0))

        if total_score < HOARD_SCORE_MIN:
            continue

        results.append({
            "branch": br, "ticker": tk, "name": r["name"],
            ...
            "no_price": no_price, "freshness": freshness,
            "dims": {
...
def render_hoarding_table(hoarding: list[dict], data_date: str, twse_ohlc: dict) -> str:
    ...
    for i, r in enumerate(hoarding, 1):
        ...
        fresh = r.get("freshness", "unknown")
        if fresh == "new":
            fresh_pill = ' <span class="pill pill-red" title="前期幾乎不買、近期高頻買超">新進場</span>'
        elif fresh == "rising":
            fresh_pill = ' <span class="pill pill-orange" title="買超頻率較前期明顯上升">升溫</span>'
        elif fresh == "routine":
            fresh_pill = ' <span class="pill pill-gray" title="前期就頻繁買超，可能是常態流量">常態流量</span>'
        else:
            fresh_pill = ""
        ...
        rows += f"""<tr>
          ...
          <td><div class="tk"><span class="tk-code">{tk}</span><span class="tk-name">{r['name']}</span></div>{co_pill}{confirm_pill}{fresh_pill}</td>
          <td>{r['branch']}</td>
...
<div class="container">
  ...
  <div id="hoarding" class="tab-panel">
    ...
    偵測分點可能悄悄吸籌的股票；僅 ≥{HOARD_SCORE_MIN} 分進榜（含新鮮度調整後）。「長線佈局」標記表示 30 日窗口內買超天數/總量同樣達標，屬於信心加分。<br>
    「新進場」= 前期幾乎沒買、近期才開始高頻買超（分數 ×1.10）；「升溫」= 買超頻率較前期明顯上升（×1.05）；
    「常態流量」= 前期就頻繁買超，可能只是分點的例行機構單（×0.85）。前期資料不足時不顯示標記、不調整分數。<br>
...
def render_email_digest(all_branches: list[dict], consensus: list[dict],
    ...
    # ── 囤貨警示（is_new 優先 → freshness=new 次之，再按分數降冪）──
    hoard_sorted = sorted(
        hoarding,
        key=lambda x: (not x.get("is_new", False), x.get("freshness") != "new", -x["score"])
    )
    rows_h = ""
    for r in hoard_sorted[:10]:
        ...
        fresh = r.get("freshness", "unknown")
        if fresh == "new":
            extra += pill_s("新進場", "#fef2f2", "#991b1b")
        elif fresh == "rising":
            extra += pill_s("升溫", "#fff7ed", "#c2410c")
        elif fresh == "routine":
            extra += pill_s("常態流量", "#f1f5f9", "#5f6368")
...
def main():
    ...
    fresh_dist = Counter(r.get("freshness", "unknown") for r in hoarding)
    print(f"\n🎯 囤貨追蹤：{len(hoarding)} 筆（其中新進榜 {n_new} 筆）")
    print(f"   新鮮度分佈：new={fresh_dist['new']} rising={fresh_dist['rising']} "
          f"steady={fresh_dist['steady']} routine={fresh_dist['routine']} unknown={fresh_dist['unknown']}")
    for r in hoarding[:5]:
